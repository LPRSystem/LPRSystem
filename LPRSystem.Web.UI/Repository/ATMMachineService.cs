...
namespace LPRSystem.Web.UI.Repository
{
    public class ATMMachineService : IATMMachineService
    {
        ...
        public async Task<List<ATMMachine>> FetchAllATMMachines()
        {
            List<ATMMachine> aTMMachines = new List<ATMMachine>();

            var response = await _httpClient.GetAsync("ATMMachine");
        ...
        public async Task<string> InsertOrUpdateATMMachine(ATMMachine atmMachine)
        {
            ...
            var responce = await _httpClient.PostAsync("ATMMachineProcess", requestatmMachine);
